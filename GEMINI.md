...
## 3-1. 메인 페이지 현재 구현 기준
* 상단 메인 배너 중앙에 **문의하기 버튼**을 배치한다. (현재는 UI만 제공)
* 메인 배너 하단에는 대학 리스트를 **카드 형태**로 노출한다.
* 대학 리스트는 `/api/university-list` API를 통해 조회한다.
* 초기 구현 대상 대학은 **가천대학교**이며, 이후 대학 수 확장을 전제로 리스트 구조를 유지한다.
